...
const SociosPage = ({ showToast }) => {
    ...
    }, [filters, page]); // Added page dependency to simplified logic, but actually we want to trigger load on page change too.

    // Effect for page change is slightly redundant if we include page in the dependency above, 
    // but usually we want search/filter changes to reset page to 1 (handled in setFilters usually or effect), 
    // and page changes to just load.
    // Let's simplify: Just one effect for loading data when dependencies change? 
    // The previous code had specific behavior for search.

    // Let's stick to the previous pattern but updated:
    /*
    useEffect(() => {
         loadSocios(page, filters.search, filters.categoria, filters.radio);
    }, [page]);
    */
    // Actually, the previous code had a specific debounce for search separate from page.
    // Let's implement a clean unified effect or keep them separate.
    // To avoid complex refactor loops, I'll keep the debounce on filters.
    ...
    return (
        <div className={styles.page}>
            <div className={styles.header}>
                <h2 className={styles.title}>Listado de Socios</h2>
            </div>
...

            {
                loading ? (
                    ...
                    <>
                        <SociosTable
                            socios={socios}
                            onSocioDeleted={() => loadSocios(page, filters.search, filters.categoria, filters.radio)}
                        />
